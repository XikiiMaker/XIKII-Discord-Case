...
import { renderDesktopSettings } from "./desktop-settings";
...
export async function renderTranslationSettings() {
  ...
  void renderDesktopSettings(existing);
  ...
  const fallbackHeading = document.createElement("h2"); fallbackHeading.textContent = "备用翻译 · LibreTranslate"; form.append(fallbackHeading);
  const fallbackNotice = document.createElement("p");
  fallbackNotice.textContent = "启用后，Qwen 失败时将当前待译消息发送至你填写的服务，不发送最近对话。链接、代码和提及会保留。备用译文缓存 1 分钟，不支持流式输出；更换服务地址后需重新填写其 Key。";
  form.append(fallbackNotice);
  const fallbackEnabled = checkbox("Qwen 失败时允许使用下方备用服务", state.settings.fallback.enabled);
  const fallbackEndpoint = text("服务完整地址（HTTPS，路径以 /translate 结尾）", state.settings.fallback.endpoint, "url");
  fallbackEndpoint.placeholder = "https://your-service.example/translate";
  const fallbackKey = text("备用 API Key（可选；留空保留已有 Key）", "", "password"); fallbackKey.autocomplete = "off";
  fallbackKey.placeholder = state.hasFallbackKey ? "已安全保存" : "未配置（服务可能无需 Key）"; fallbackKey.disabled = !state.encryptionAvailable;
  const save = document.createElement("button"); save.type = "submit"; save.textContent = "保存翻译设置";
  const remove = document.createElement("button"); remove.type = "button"; remove.textContent = "删除 Qwen Key";
  const removeFallback = document.createElement("button"); removeFallback.type = "button"; removeFallback.textContent = "删除备用 Key";
  const clear = document.createElement("button"); clear.type = "button"; clear.textContent = "清空翻译缓存";
  form.append(save, remove, removeFallback, clear); section.append(form);
  if (!state.encryptionAvailable) status.textContent = "系统安全存储不可用，无法保存或使用 API Key。";
  let saving = false;
  async function persist(removeKey?: "qwen" | "fallback") {
    if (saving) return;
    saving = true; save.disabled = remove.disabled = removeFallback.disabled = true;
    try {
      const settings: TranslationSettings = { ...state.settings, consent: consent.checked, dmEnabled: dm.checked,
        target: target.value as TranslationSettings['target'],
        ...
        fallback: { enabled: fallbackEnabled.checked, endpoint: fallbackEndpoint.value.trim() } };
      const result = await ipc.invoke("translation:save", { settings,
        ...(removeKey === "qwen" ? { key: "" } : key.value.trim() ? { key: key.value.trim() } : {}),
        ...(removeKey === "fallback" ? { fallbackKey: "" } : fallbackKey.value.trim() ? { fallbackKey: fallbackKey.value.trim() } : {}) }) as Reply<TranslationState>;
      if (!result.ok) throw new Error(result.error);
      state = result.value; key.value = ""; key.placeholder = state.hasKey ? "已安全保存" : "尚未配置";
      fallbackKey.value = ""; fallbackKey.placeholder = state.hasFallbackKey ? "已安全保存" : "未配置（服务可能无需 Key）";
      status.textContent = removeKey ? (removeKey === "qwen" ? "Qwen Key 已删除。" : "备用 Key 已删除。") : "设置已保存，立即生效。";
    } catch (error) { status.textContent = error instanceof Error ? error.message : "保存失败。"; }
    finally { saving = false; save.disabled = remove.disabled = removeFallback.disabled = false; }
  }
