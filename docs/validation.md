...
- `npm test`：TypeScript 编译、43 项 Node/DOM 测试和 oxlint。
...
- 模拟联调验证 29 项：密钥安全存储、直接发送、流式与原文显示、频道开关及非当前频道点击不跳转、草稿/路由防误发、备用引擎与独立密钥、宽窄窗口无遮挡、中文行高、客户端通知设置保存等。证据：`cache/evidence/smoke-result.json`。
- `npm run test:slate`：真实 React 19.3.0、Slate 0.126.2、slate-react 0.126.4，验证 7 项富文本场景。使用正式翻译 preload/IPC，人工翻译响应，不访问 Discord。
- 中文语言包按英文全部字符串键检查覆盖；正式设置 preload 验证隐私、权限等选项均为中文。
...
- Windows x64 ZIP、Squirrel EXE/MSI 已构建。已检查 ASAR 包含中英翻译模块且排除测试与运行缓存；上游更新关闭。打包程序 `--version` 返回 `XIKII Discord Case v0.1.2, stable build`，退出码 0。此检查不替代真实登录与干净环境安装/卸载验收，操作步骤见 [Windows 测试说明](windows-test.md)。
...
![中文隐私选项](images/client-privacy-chinese.png)

![固定右侧开关列](images/translation-fixed-column.png)

![保留提及和表情的 Slate 测试](images/slate-rich-composer.png)

![翻译设置](images/translation-settings.png)

## 必须区分的验证边界

页面综合测试使用原生 contenteditable；另有真实 Slate/React 编辑器测试。两者都不包含 Discord 当前定制的编辑器代码。因此这些结果证明本地流程和 IPC 能协作，不等于真实 Discord 兼容性已通过。
...
工具没有向真实 Discord 联系人发送测试消息。用户已在真实频道及私信中测试普通文本翻译和发送，并报告翻译基本准确、功能基本实现。0.1.2 根据截图修正 UI。最新开关布局、真实通知的横幅/声音/跳转、实际 Windows 开机启动及结构化 Slate 输入仍需人工复核；本地模拟通过不替代这些结果。
...
打包程序入口对照：对 0.1.2 与 0.1.3 分别执行 `xikii-discord-case.exe --squirrel-firstrun --version`，旧版输出提前退出提示，新版正常输出 `XIKII Discord Case v0.1.3`，两次均未启动 Discord 窗口。此项证明首次运行参数已能进入正常程序入口；不替代实际安装验收。
