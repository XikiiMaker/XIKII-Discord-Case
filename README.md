...
- **成本控制**：译文缓存加密保存在本机，切换频道或重启客户端都不会重复翻译；另有重复请求合并、并发队列、按会话隔离的上下文。上下文默认关闭。
- **桌面功能**：保留登录会话，可选开机启动，桌面通知设置、任务栏闪烁和托盘未读提示。
- **备用引擎**：可选自行配置的 LibreTranslate 服务，默认关闭；见 [说明](docs/fallback.md)。

![频道中原文下方显示中文译文，频道名右侧是翻译开关](docs/images/demo-channel.png)

![私信列表每个会话名称右侧都有独立的翻译开关](docs/images/demo-dm.png)

*截图取自实际运行的客户端，其中的用户名与对话均为示例数据，不含真实聊天记录。*
...
![翻译设置窗口，可选择地域、模型、发送语言和上下文条数](docs/images/demo-translation-settings.png)
...
API Key 仅在 Electron 主进程使用，通过 `safeStorage` 加密保存在本机，不向 Discord 页面暴露。上下文只在内存中管理。备用引擎独立配置，不接收 Qwen Key 或对话上下文。

为避免重复付费，已完成的译文会经 `safeStorage` 加密后写入本机 `translation-cache.json`（Windows 位于 `%APPDATA%\XIKII Discord Case`），最长保留 7 天，随文件一同保存的还有对应原文。系统安全存储不可用时不会写入任何缓存文件。在「翻译设置 → 清空缓存」可随时删除该文件；备用引擎的译文只在内存中短暂保留，不会落盘。
